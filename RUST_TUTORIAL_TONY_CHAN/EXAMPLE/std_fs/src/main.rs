...
altre operazioni correlate ai file direttamente dal codice:
    ...
    @std@fs_(esempio per l'utillizzo degli strumenti per la gestione del file system)

Qui, use std::fs importa il modulo fs, e poi utilizziamo fs::write per scrivere una
stringa in un file e fs::read_to_string per leggerne il contenuto.
*/

use std::fs;            //gestione del file system

fn main() {
    println!("PROGETTO PER LA GESTIONE DEL FILE SISTEM !");
