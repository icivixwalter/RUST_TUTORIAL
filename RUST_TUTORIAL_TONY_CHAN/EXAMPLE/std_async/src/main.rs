...
async è sinonomi di concorrenza:

"Concorrenza con thread"

"Uso di Mutex e Arc per sincronizzazione"

"Parallelo con std::thread"
...
// Importa Arc (Reference Counted) per condividere dati tra thread in modo sicuro,
// Mutex per proteggere l'accesso concorrente ai dati,
// e thread per creare ed eseguire thread separati.
...
fn main() {
    ...
    // Stampa un messaggio di intestazione (nota: il nome "STD::Async" è solo nel messaggio,
    // ma in realtà non viene usata async/await né librerie asincrone).
    println!("PROGETTO DI GESTIONE DELLA LIBRERIA STD::Async!");

    // Crea un contatore inizializzato a 0, protetto da Mutex
    // e incapsulato in Arc per la condivisione sicura tra thread.
    let contatore = Arc::new(Mutex::new(0));

    // Vettore per memorizzare gli handle dei thread creati.
    let mut handles = vec![];

    // Crea 10 thread concorrenti.
    for _ in 0..10 {
        // Clona l'Arc per ottenere una nuova referenza condivisa al contatore.
        let contatore = Arc::clone(&contatore);

        // Crea un nuovo thread.
        let handle = thread::spawn(move || {
            // Ottiene il lock sul Mutex per accedere in modo sicuro al contatore.
            let mut num = contatore.lock().unwrap();
            println!("il lock del contatore! {}", num);


            // Incrementa il contatore di 1.
            *num += 1;
            // Il lock viene rilasciato automaticamente al termine dello scope.
        });

        // Salva l'handle del thread per poterlo aspettare più tardi.
        handles.push(handle);
    }

    // Attende che tutti i thread terminino l'esecuzione.
    for handle in handles {
        handle.join().unwrap(); // join() blocca fino alla fine del thread.
    }

    // Dopo che tutti i thread hanno completato, stampa il valore del contatore,
    // che dovrebbe essere 10 se tutti i thread hanno incrementato correttamente.
